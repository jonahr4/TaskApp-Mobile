                ...
                >
                    ...
                    {aiConsentKnown && !aiConsentGranted && (
                        <View style={[styles.inputCard, { marginBottom: Spacing.md }]}>
                            <Text style={[styles.fieldLabel, { marginTop: 0 }]}>AI DATA SHARING</Text>
                            <Text style={{ fontSize: FontSize.sm, color: C.textSecondary, lineHeight: 20 }}>
                                To use AI Reminder, TaskApp sends the text you type, your timezone, and your task group names to our AI provider endpoint at the-task-app.vercel.app.
                            </Text>
                            <Text style={{ fontSize: FontSize.sm, color: C.textSecondary, lineHeight: 20, marginTop: 4 }}>
                                This data is used only to parse your reminder into structured tasks. Choose Continue to allow sending this data.
                            </Text>
                            <View style={{ flexDirection: "row", gap: 10, marginTop: Spacing.sm }}>
                                <TouchableOpacity
                                    style={[styles.parseBtn, { flex: 1, backgroundColor: C.bgElevated, borderWidth: 1, borderColor: C.borderLight }]}
                                    onPress={() => setAiConsent(false)}
                                >
                                    <Text style={[styles.parseBtnText, { color: C.textPrimary }]}>Not now</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    style={[styles.parseBtn, { flex: 1 }]}
                                    onPress={() => setAiConsent(true)}
                                >
                                    <Text style={styles.parseBtnText}>Continue</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    )}
